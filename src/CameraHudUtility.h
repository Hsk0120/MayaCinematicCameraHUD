...
void convert_unitToTime(const MTime::Unit &unit, double &framesPerSecond)
{
	switch (unit) {
	case MTime::kGames:
...
void convert_secondsToTime(double seconds, int& hours, int& minutes, int& secondsOut, int& frames, double framesPerSecond, MTime::Unit unit) {
...
// フレーム数の取得と+fの文字列足し
char* get_frame(static char result[64], double frame)
{
    static char str[64];
    static char num[64];
...
MString get_camera_name(std::string camName){
    std::size_t pos = camName.find("Shape");  // "shape" の位置を検索
    if (pos != std::string::npos) {  // "shape" が見つかった場合
        camName.erase(pos);  // "shape" 以降の文字列を削除
    }
...
// テキスト位置 左下
MPoint get_viewport_leftBottom(int num, int width, int height, double fontSize, MPoint offset, MPoint margin, MPoint maskOffset) {
    return MPoint(
...
// テキスト位置 左上
MPoint get_viewport_leftTop(int num, int width, int height, double fontSize, MPoint offset, MPoint margin, MPoint maskOffset) {
    return MPoint(
        offset.x + margin.x + maskOffset.x,
...
// テキスト位置 右下
MPoint get_viewport_rightBottom(int num, int width, int height, double fontSize, MPoint offset, MPoint margin, MPoint maskOffset) {
    return MPoint(
...
// テキスト位置 右上
MPoint get_viewport_rightTop(int num, int width, int height, double fontSize, MPoint offset, MPoint margin, MPoint maskOffset) {
    return MPoint(
        width - offset.x - margin.x - maskOffset.x,
        height - maskOffset.y - offset.y - (margin.y + fontSize) * (num + 1),
        0);
...
// テキスト位置 真ん中下
MPoint get_viewport_centerBottom(int num, int width, int height, double fontSize, MPoint offset, MPoint margin, MPoint maskOffset) {
    return MPoint(
        width / 2,
        offset.y + maskOffset.y + (margin.y + fontSize) * num,
        0);
...
// テキスト位置 真ん中上
MPoint get_viewport_centerTop(int num, int width, int height, double fontSize, MPoint offset, MPoint margin, MPoint maskOffset) {
    return MPoint(
        width / 2,
        height - maskOffset.y - offset.y - (margin.y + fontSize) * (num + 1),
        0);
